...
/**
 * Shared worksite-geofence editor. Branch and agency geofences are the same kind
 * of record (a branch row with a GPS pin) but are managed as separate modules, so
 * this component renders one of the two views based on `kind` and filters the
 * shared /branches list accordingly. Wording is fixed per kind (no branch/agency
 * term swap) precisely because the two now live on distinct pages.
 */
export function GeofenceManager({ kind }: { kind: "branch" | "agency" }) {
  ...
  const rows = branches.filter((b) => (isAgency ? b.is_agency : !b.is_agency));

  const title = isAgency ? "Agency Geofence" : "Branch Geofence";
  const noun = isAgency ? "agency worksite" : "branch";
  const nounPlural = isAgency ? "agency worksites" : "branches";
  const colLabel = isAgency ? "Agency" : "Branch";

  if (me && !canManage) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
        You don&apos;t have permission to manage {nounPlural} geofences.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={title}
        description={`Set each ${noun}'s GPS pin and the allowed radius. Web check-ins are measured against the employee's ${noun} pin and flagged On-site / Outside. A ${noun} with no pin is simply not geofenced yet.`}
      />
      ...
      <TableShell>
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading && (
              <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-400">Loading {nounPlural}…</td></tr>
            )}
            {!isLoading && rows.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-500">No {nounPlural} found for this company.</td></tr>
            )}
