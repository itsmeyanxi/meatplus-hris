...
export default function AccessRequestsPage() {
  ...
  if (!canView) {
    return (
      <AppCard className="mx-auto max-w-md text-center">
        ...
        <Link href="/dashboard" className="mt-6 inline-flex rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-slate-800">
          Back to dashboard
        </Link>
      </AppCard>
    );
  ...
  const queueCards = (["supervisor", "hr", "it"] as const)
    ...
    .map((s) => ({ label: STAGE_QUEUE_LABEL[s], pending: stats?.queues?.[s] ?? 0 }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {statCards.map((card) => (
          <div key={card.label} className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] backdrop-blur">
      ...
      {queueCards.length > 0 && (
        <div>
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Approver Queues</h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {queueCards.map((q) => (
              <div key={q.label} className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] backdrop-blur">
                <p className="text-sm font-medium text-slate-700">{q.label}</p>
                <div className="mt-2 flex items-baseline gap-1.5">
                  <span className="text-2xl font-semibold text-slate-900">{q.pending}</span>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 text-left text-slate-600">
              <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Requester</th>
              <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Request type</th>
              <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Stage</th>
              <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Status</th>
              <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide">Submitted</th>
              <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide">Actions</th>
            </tr>
          </thead>
          <tbody>
            {listLoading ? (
              <tr><td colSpan={COL_SPAN} className="px-4 py-16 text-center text-sm text-slate-500">Loading…</td></tr>
            ) : requests.length === 0 ? (
              <tr>
                <td colSpan={COL_SPAN} className="px-4 py-16 text-center">
                  ...
                  <p className="mt-1 text-sm text-slate-500">Submitted requests will appear here for review.</p>
                </td>
              </tr>
            ) : (
              requests.map((r) => (
                <>
                  <tr key={r.id} className="border-t border-slate-100 transition hover:bg-slate-50/70">
                    ...
                    <td className="px-4 py-3 text-slate-700">{humanize(r.request_type)}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        {r.status === "pending" && !canActOnStage(r.current_stage) && (
                          <span className="text-xs text-slate-400">Awaiting {humanize(r.current_stage)}</span>
                        )}
                      </div>
                    </td>
                  </tr>
