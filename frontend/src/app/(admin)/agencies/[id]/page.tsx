...
import { getAgencyToday, addAgencyEmployee, getAgencyAttendanceData, type AgencyTodayRow, type AgencyAttendanceData } from "@/lib/agencies";
...
function buildPrintHtml(d: AgencyAttendanceData, scopeLabel: string): string {
  // Group day-records by worker so each person gets their own block + subtotals.
...
export default function AgencyDetailPage() {
  ...
  const printPdf = async () => {
    ...
    try {
      const d = await getAgencyAttendanceData({ branchId, dateFrom, dateTo, employeeId: workerId || undefined });
      const worker = workerId ? (data?.employees ?? []).find((e) => e.id === workerId) : undefined;
      const scope = worker ? `Worker: ${worker.name} (${worker.employee_no})` : "All workers";
      const win = window.open("", "_blank");
      if (!win) { setErr("Allow pop-ups for this site to generate the PDF."); return; }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Pull this {term.singular}&rsquo;s time logs for a date range — Excel with Summary, Daily Shifts (shift start/end + hours) and raw Punch Records.
        </p>
        <div className="mt-4 flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <select className={inputCls} value={workerId} onChange={(e) => setWorkerId(e.target.value ? Number(e.target.value) : "")}>
              <option value="">All workers (whole {term.singular})</option>
              {rows.map((e) => (
