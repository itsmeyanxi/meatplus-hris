...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {/* DESKTOP SIDEBAR */}
      <aside className="hidden border-r border-slate-200/80 bg-white/90 dark:border-slate-700/80 dark:bg-slate-900/90 px-3.5 py-6 backdrop-blur lg:flex lg:min-h-screen lg:flex-col justify-between transition-all duration-300">
        <div className="space-y-6">
          {/* Header & Toggle Button */}
          <div className="flex items-center justify-between gap-1.5 px-1">
            {!isCollapsed && (
              <div className="transition-opacity duration-200 min-w-0">
                <h1 className="text-base font-semibold tracking-tight text-slate-900 dark:text-slate-100 truncate">Meatplus HRIS</h1>
                {isItAccount && switchableCompanies.length > 1 ? (
                  <div className="relative">
                    <button
                      onClick={() => setCompanyOpen((o) => !o)}
                      className="mt-0.5 flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 truncate max-w-[150px]"
                    >
                        ...
                        {switchableCompanies.map((c) => (
                          <button
                            ...
                            className={`flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-slate-50 dark:hover:bg-slate-700 ${
                              c.id === data?.user.active_company?.id ? "font-semibold text-slate-900 dark:text-slate-100" : "text-slate-600 dark:text-slate-400"
                            }`}
                          >
                            {switchingId === c.id ? (
                  ...
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400 truncate max-w-[150px]">{companyName}</p>
                )}
              </div>
            )}
            <button
              ...
              className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white mx-auto transition shrink-0"
                ...
                <Link
                  ...
                  className={
                    active
                      ...
                      : `flex items-center rounded-xl py-2 text-sm font-medium text-slate-700 dark:text-slate-300 transition hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white ${
                          isCollapsed ? "justify-center px-0" : "px-2.5 gap-2.5"
                        }`
                  }
                >
        ...
        <div className="mt-auto space-y-2.5">
          ...
          <div className={`border border-slate-200 bg-slate-50 p-2.5 transition-all dark:border-slate-700 dark:bg-slate-800 ${isCollapsed ? "rounded-xl text-center" : "rounded-xl"}`}>
            {isCollapsed ? (
              <button
              ...
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Signed in</p>
                <p className="mt-0.5 break-all text-xs font-medium text-slate-900 dark:text-slate-100 line-clamp-2">
      ...
      <div className="min-w-0">
        ...
        {/* Desktop top bar */}
        <div className="sticky top-0 z-20 hidden items-center justify-end gap-1 border-b border-slate-200/80 bg-white/80 dark:border-slate-700/80 dark:bg-slate-900/80 px-6 py-2 backdrop-blur lg:flex">
          <button
            type="button"
            onClick={() => setIsDark((d) => !d)}
            className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white transition"
            title={themeMounted && isDark ? "Switch to light mode" : "Switch to dark mode"}
          >
            {themeMounted && isDark ? (
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 dark:border-slate-700/80 dark:bg-slate-900/90 backdrop-blur lg:hidden">
          <div className="space-y-3 px-4 py-3 sm:px-6">
              ...
              {visibleNav.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={
                      active
                        ? "whitespace-nowrap rounded-full bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm dark:bg-white dark:text-slate-900"
                        : "whitespace-nowrap rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                    }
                  >
                    {item.label}
                  </Link>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 break-words">{isLoading ? "Loading…" : data?.user.email}</p>
          </div>
        </header>
...
function AccessDenied() {
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white/90 p-8 text-center shadow-[0_1px_2px_rgba(15,23,42,0.04)] backdrop-blur dark:border-slate-700 dark:bg-slate-900/90">
