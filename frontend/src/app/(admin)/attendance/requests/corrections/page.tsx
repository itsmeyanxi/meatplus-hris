              ...
              <tr key={r.id}>
                ...
                <td className="px-3 py-2 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/attendance/requests/corrections/${r.id}`} className="rounded-md border border-slate-300 px-2 py-1 text-xs font-medium hover:bg-slate-100">View</Link>
                    {r.status === "pending" && (
                      <>
                        {canApprove && (
                          <>
