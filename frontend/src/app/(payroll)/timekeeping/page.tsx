...
export default function PayrollTimekeepingPage() {
  ...
  const rows = (data?.items ?? []).filter((r) => (onlyFloating ? r.floating_total > 0 : true));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <button onClick={() => refetch()} className="rounded-lg bg-slate-900 px-3.5 py-2 text-sm font-semibold text-white hover:bg-slate-800">Apply</button>
      ...
      {period && (
        <p className="text-xs text-slate-400">Cutoff: {period.from} → {period.to}</p>
      )}
